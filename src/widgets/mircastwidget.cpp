...
void MircastWidget::togglePopup()
{
    ...
    if (isVisible()) {
        ...
        PlayerEngine *engine = static_cast<PlayerEngine *>(m_pEngine);
        if (engine && engine->playlist().currentInfo().url.isLocalFile()) {
            m_dlnaContentServer->setDlnaFileName(engine->playlist().currentInfo().url.toLocalFile());
            m_sLocalUrl = m_dlnaContentServer->getBaseUrl() + QFileInfo(engine->playlist().currentInfo().url.toLocalFile()).fileName().toLatin1();
            QRCodeWidget *qrcode = new QRCodeWidget(m_sLocalUrl, this);
            qrcode->exec();
        }
...
void MircastWidget::startDlnaTp(ItemWidget *item)
{
    ...
    if(!m_dlnaContentServer)
    {
        ...
        if(pEngine && pEngine->playlist().currentInfo().url.isLocalFile()) {
            ...
            QString fileName = QFileInfo(pEngine->playlist().currentInfo().url.toLocalFile()).fileName();
            m_sLocalUrl = m_dlnaContentServer->getBaseUrl() + QString::fromLatin1(QUrl::toPercentEncoding(fileName));
        } else {
