    ...
    if (!m_pTitlebarMenu) {
        ...
        if (!CompositingManager::isPadSystem()) {
            ...
            }
            {
                ...
                DEF_ACTION_CHECKED_GROUP(tr("2.5x"), ActionKind::TowPointFiveTimes, pActionGroup);
                DEF_ACTION_CHECKED_GROUP(tr("3.0x"), ActionKind::Treble, pActionGroup);
                DEF_ACTION_CHECKED_GROUP(tr("10.0x"), ActionKind::TenTimes, pActionGroup);
                pParent->addMenu(pMenu);
                pMenu->setEnabled(false);
