...
namespace {
class AuxProgressTrackWidget final : public QWidget
{
public:
    explicit AuxProgressTrackWidget(QWidget *parent = nullptr)
        : QWidget(parent)
    {
...
protected:
    void paintEvent(QPaintEvent *) override
    {
        ...
        QColor fill = property("fill").value<QColor>();
        if (!fill.isValid()) {
            fill = QColor(230, 230, 230);
        }

        QColor border = property("border").value<QColor>();

        const QRectF r = QRectF(rect()).adjusted(0.5, 0.5, -0.5, -0.5);
        const qreal radius = qMin<qreal>(r.height() / 2.0, 4.0);
    ...
    : DMainWindow(nullptr)
{
    ...
    m_pAuxProgressBar = new QFrame(this);
    m_pAuxProgressBar->setObjectName("aux_progress_bar");
    m_pAuxProgressBar->setAttribute(Qt::WA_NoSystemBackground);
    m_pAuxProgressBar->setAttribute(Qt::WA_TranslucentBackground);
    m_pAuxProgressBar->setFocusPolicy(Qt::NoFocus);
    m_pAuxProgressBar->hide();

    m_pAuxProgressOpacity = new QGraphicsOpacityEffect(m_pAuxProgressBar);
    m_pAuxProgressOpacity->setOpacity(0.45);
    m_pAuxProgressBar->setGraphicsEffect(m_pAuxProgressOpacity);

    m_pAuxProgressSlider = new DMRSlider(m_pAuxProgressBar);
    m_pAuxProgressSlider->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    m_pAuxProgressSlider->slider()->setOrientation(Qt::Horizontal);
    m_pAuxProgressSlider->slider()->setFocusPolicy(Qt::NoFocus);
    m_pAuxProgressSlider->slider()->setObjectName("aux_progress_slider");
    m_pAuxProgressSlider->slider()->setRange(0, 100);
    m_pAuxProgressSlider->setValue(0);
    m_pAuxProgressSlider->setEnableIndication(false);

    m_pAuxProgressTrackBg = new AuxProgressTrackWidget(m_pAuxProgressBar);
    m_pAuxProgressTrackBg->setObjectName("aux_progress_track_bg");

    m_pAuxProgressTime = new QLabel(m_pAuxProgressBar);
    m_pAuxProgressTime->setObjectName("aux_progress_time");
    m_pAuxProgressTime->setAlignment(Qt::AlignVCenter | Qt::AlignRight);
    m_pAuxProgressTime->setAttribute(Qt::WA_TransparentForMouseEvents);
    DFontSizeManager::instance()->bind(m_pAuxProgressTime, DFontSizeManager::T8);

    QHBoxLayout *auxLayout = new QHBoxLayout(m_pAuxProgressBar);
    auxLayout->setContentsMargins(10, 0, 10, 0);
    auxLayout->setSpacing(0);
    auxLayout->addWidget(m_pAuxProgressSlider, 1);
    m_pAuxProgressBar->setLayout(auxLayout);
    m_pAuxProgressBar->setFixedHeight(18);
    m_pAuxProgressSlider->setFixedHeight(10);

    m_pAuxProgressBar->installEventFilter(this);
    m_pAuxProgressSlider->installEventFilter(this);
    m_pAuxProgressSlider->slider()->installEventFilter(this);
    m_pAuxProgressTime->installEventFilter(this);
    m_pAuxProgressTime->raise();
    if (m_pAuxProgressTrackBg) {
        m_pAuxProgressTrackBg->stackUnder(m_pAuxProgressSlider);
    }

    applyAuxProgressSliderStyle();
    connect(DGuiApplicationHelper::instance(), &DGuiApplicationHelper::themeTypeChanged, this, &MainWindow::applyAuxProgressSliderStyle);
    connect(m_pEngine, &PlayerEngine::fileLoaded, this, [this]() {
        if (!m_pAuxProgressSlider) {
            return;
        }
        m_pAuxProgressSlider->slider()->setRange(0, static_cast<int>(m_pEngine->duration()));
        m_pAuxProgressSlider->slider()->setValue(static_cast<int>(m_pEngine->elapsed()));
        updateAuxProgressBarTime();
        applyAuxProgressSliderStyle();
        updateAuxProgressBarVisibility();
    });
    connect(m_pEngine, &PlayerEngine::updateDuration, this, [this]() {
        if (!m_pAuxProgressSlider || !m_pAuxProgressSlider->slider() || !m_pEngine) {
            return;
        }
        const qint64 duration = m_pEngine->duration();
        if (duration > 0) {
            m_pAuxProgressSlider->slider()->setRange(0, static_cast<int>(duration));
        }
        updateAuxProgressBarTime();
        applyAuxProgressSliderStyle();
        updateAuxProgressBarVisibility();
    });
    connect(m_pEngine, &PlayerEngine::elapsedChanged, this, [this]() {
        if (!m_pAuxProgressSlider || m_bAuxProgressDragging) {
            return;
        }
        m_pAuxProgressSlider->slider()->setValue(static_cast<int>(m_pEngine->elapsed()));
        updateAuxProgressBarTime();
    });
    connect(m_pEngine, &PlayerEngine::stateChanged, this, [this]() {
        applyAuxProgressSliderStyle();
        updateAuxProgressBarVisibility();
    });
    connect(&Settings::get(), &Settings::baseChanged, this, [this](const QString &key, const QVariant &) {
        if (key == "base.play.aux_progress_bar") {
            updateAuxProgressBarVisibility();
        }
    });
...
bool MainWindow::event(QEvent *pEvent)
{
    ...
    if (pEvent->type() == QEvent::Polish || pEvent->type() == QEvent::PolishRequest || pEvent->type() == QEvent::Show) {
        if (!property("_aux_progress_polished").toBool()) {
            setProperty("_aux_progress_polished", true);
                               ...
                               QList<QVariant> args, bool bIsShortcut)
{
    ...
    case ActionFactory::ActionKind::ZeroPointFiveTimes: {
        ...
        if (m_pEngine->state() != PlayerEngine::CoreState::Idle) {
            ...
            Settings::get().setInternalOption("play_speed", m_dPlaySpeed);
        }
        break;
    }
    case ActionFactory::ActionKind::OneTimes: {
    ...
    case ActionFactory::ActionKind::TowPointFiveTimes: {
        qDebug() << "ActionFactory::ActionKind::TowPointFiveTimes";
        if (m_pEngine->state() != PlayerEngine::CoreState::Idle) {
            qDebug() << "ActionFactory::ActionKind::TowPointFiveTimes, m_pEngine->state() != PlayerEngine::CoreState::Idle";
            m_dPlaySpeed = 2.5;
            m_pEngine->setPlaySpeed(m_dPlaySpeed);
            m_pCommHintWid->updateWithMessage(tr("Speed: %1x").arg(m_dPlaySpeed));
            Settings::get().setInternalOption("play_speed", m_dPlaySpeed);
    ...
    case ActionFactory::ActionKind::Treble: {
        qDebug() << "ActionFactory::ActionKind::Treble";
        if (m_pEngine->state() != PlayerEngine::CoreState::Idle) {
            qDebug() << "ActionFactory::ActionKind::Treble, m_pEngine->state() != PlayerEngine::CoreState::Idle";
            m_dPlaySpeed = 3.0;
            m_pEngine->setPlaySpeed(m_dPlaySpeed);
            m_pCommHintWid->updateWithMessage(tr("Speed: %1x").arg(m_dPlaySpeed));
            Settings::get().setInternalOption("play_speed", m_dPlaySpeed);
    ...
    case ActionFactory::ActionKind::TenTimes: {
        qDebug() << "ActionFactory::ActionKind::TenTimes";
        if (m_pEngine->state() != PlayerEngine::CoreState::Idle) {
            qDebug() << "ActionFactory::ActionKind::TenTimes, m_pEngine->state() != PlayerEngine::CoreState::Idle";
            m_dPlaySpeed = 10.0;
            m_pEngine->setPlaySpeed(m_dPlaySpeed);
            m_pCommHintWid->updateWithMessage(tr("Speed: %1x").arg(m_dPlaySpeed));
            Settings::get().setInternalOption("play_speed", m_dPlaySpeed);
...
void MainWindow::updateProxyGeometry()
{
    ...
    if (m_pAuxProgressBar && m_pAuxProgressBar->isVisible()) {
        updateAuxProgressBarGeometry();
    }
}

void MainWindow::updateAuxProgressBarGeometry()
{
    if (!m_pAuxProgressBar) {
        return;
    }

    const int gap = (windowState() & Qt::WindowFullScreen) ? 0 : 2;
    const int h = m_pAuxProgressBar->height();
    m_pAuxProgressBar->setGeometry(0, rect().height() - h - gap, rect().width(), h);
    m_pAuxProgressBar->raise();

    if (m_pAuxProgressTime) {
        const QFontMetrics fm(m_pAuxProgressTime->font());
        const int timeW = fm.horizontalAdvance(QStringLiteral("00:00:00 / 00:00:00")) + 6;
        const int padR = 10;
        m_pAuxProgressTime->setGeometry(qMax(0, m_pAuxProgressBar->width() - padR - timeW), 0, timeW, h);
        m_pAuxProgressTime->raise();
    }

    if (m_pAuxProgressTrackBg && m_pAuxProgressSlider && m_pAuxProgressSlider->slider()) {
        auto *sliderWidget = m_pAuxProgressSlider->slider();
        const QRect sliderRect = QRect(sliderWidget->mapTo(m_pAuxProgressBar, QPoint(0, 0)), sliderWidget->size());
        const int grooveH = 4;
        const int y = sliderRect.y() + (sliderRect.height() - grooveH) / 2;
        int x = sliderRect.x();
        int w = sliderRect.width();
        if (m_pAuxProgressTime) {
            w = qMax(1, m_pAuxProgressTime->x() - x - 6);
        }
        m_pAuxProgressTrackBg->setGeometry(x, y, w, grooveH);
        m_pAuxProgressTrackBg->stackUnder(m_pAuxProgressSlider);
        m_pAuxProgressTrackBg->show();
    }
}

void MainWindow::updateAuxProgressBarTime()
{
    if (!m_pAuxProgressTime || !m_pEngine) {
        return;
    }

    const qint64 duration = m_pEngine->duration();
    const qint64 elapsed = m_pEngine->elapsed();
    const QString text = QString("%1 / %2")
                             .arg(utils::Time2str(qMax<qint64>(0, qMin(elapsed, duration > 0 ? duration : elapsed))))
                             .arg(utils::Time2str(qMax<qint64>(0, duration)));
    m_pAuxProgressTime->setText(text);
}

void MainWindow::applyAuxProgressSliderStyle()
{
    if (!m_pAuxProgressSlider || !m_pAuxProgressSlider->slider()) {
        return;
    }

    auto *slider = m_pAuxProgressSlider->slider();
    const auto highlightBase = palette().color(QPalette::Highlight);
    QColor played = highlightBase.lighter(125);
    played.setAlpha(255);

    QColor playedHover = played.lighter(110);
    playedHover.setAlpha(255);

    QColor track = QColor(230, 230, 230);
    track.setAlpha(255);

    QColor grooveBorder = QColor(0, 0, 0, 120);
    QColor timeText = QColor(255, 255, 255, 230);

    const int grooveH = 4;
    const int r = grooveH / 2;
    const int handleW = 10;
    const int handleH = 10;
    const int handleR = handleH / 2;
    const int handleMarginV = (handleH - grooveH) / 2;

    const auto toRgba = [](const QColor &c) -> QString {
        return QString("rgba(%1,%2,%3,%4)").arg(c.red()).arg(c.green()).arg(c.blue()).arg(c.alpha());
    };

    const QString qss = QString(
                            "QSlider#aux_progress_slider::groove:horizontal{"
                            "height:%1px;background:%2;border-radius:%3px;border:1px solid %10;}"
                            "QSlider#aux_progress_slider::sub-page:horizontal{"
                            "background:%4;border-radius:%3px;}"
                            "QSlider#aux_progress_slider::add-page:horizontal{"
                            "background:%2;border-radius:%3px;}"
                            "QSlider#aux_progress_slider::handle:horizontal{"
                            "width:%5px;height:%6px;margin:-%7px 0;border-radius:%8px;background:%4;}"
                            "QSlider#aux_progress_slider::handle:horizontal:hover{"
                            "background:%9;}"
                        )
                            .arg(grooveH)
                            .arg(toRgba(track))
                            ...
                            .arg(handleMarginV)
                            .arg(handleR)
                            .arg(toRgba(playedHover))
                            .arg(toRgba(grooveBorder));

    if (slider->styleSheet() != qss) {
        slider->setStyleSheet(QString());
        slider->setStyleSheet(qss);
    }

    if (m_pAuxProgressTime) {
        m_pAuxProgressTime->setStyleSheet(QString("color:%1;").arg(toRgba(timeText)));
    }

    if (m_pAuxProgressTrackBg) {
        QColor trackFill(230, 230, 230, 255);
        QColor trackBorder(0, 0, 0, 120);
        m_pAuxProgressTrackBg->setProperty("fill", trackFill);
        m_pAuxProgressTrackBg->setProperty("border", trackBorder);
        m_pAuxProgressTrackBg->update();
    }
}

void MainWindow::updateAuxProgressBarVisibility()
{
    if (!m_pAuxProgressBar || !m_pEngine || !m_pToolbox) {
        return;
    }

    const bool enabled = Settings::get().internalOption("aux_progress_bar").toBool();
    const bool shouldShow = enabled &&
                            !m_bMiniMode &&
                            (m_pEngine->state() != PlayerEngine::Idle) &&
                            !m_pToolbox->isVisible();
    ...
    updateAuxProgressBarTime();
    applyAuxProgressSliderStyle();
    m_pAuxProgressBar->show();
}

bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    if (watched == m_pToolbox && (event->type() == QEvent::Show || event->type() == QEvent::Hide)) {
        updateAuxProgressBarVisibility();
    }

    if (m_pAuxProgressSlider && m_pAuxProgressSlider->slider()) {
        auto *slider = m_pAuxProgressSlider->slider();
        if ((watched == m_pAuxProgressSlider || watched == slider) &&
            (event->type() == QEvent::MouseButtonPress ||
             event->type() == QEvent::MouseMove ||
             event->type() == QEvent::MouseButtonRelease)) {
            auto *mouseEvent = static_cast<QMouseEvent *>(event);
            if (mouseEvent->button() == Qt::LeftButton || (event->type() == QEvent::MouseMove && (mouseEvent->buttons() & Qt::LeftButton))) {
...
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
                const int mouseX = mouseEvent->pos().x();
#else
                const int mouseX = mouseEvent->position().toPoint().x();
#endif
                const int grooveWidth = qMax(1, groove.width());
                const int pos = qBound(0, mouseX - groove.x(), grooveWidth);
                const int value = QStyle::sliderValueFromPosition(slider->minimum(), slider->maximum(), pos, grooveWidth, opt.upsideDown);
                const auto setTimeText = [this](int seconds) {
                    if (!m_pAuxProgressTime || !m_pEngine) {
                        return;
                    }
                    const qint64 duration = m_pEngine->duration();
                    const QString text = QString("%1 / %2")
                                             .arg(utils::Time2str(qMax<qint64>(0, qMin<qint64>(static_cast<qint64>(seconds), duration > 0 ? duration : seconds))))
                                             .arg(utils::Time2str(qMax<qint64>(0, duration)));
                    m_pAuxProgressTime->setText(text);
                };

                if (event->type() == QEvent::MouseButtonPress) {
                    m_bAuxProgressDragging = true;
                ...
                if (event->type() == QEvent::MouseMove && m_bAuxProgressDragging) {
                    slider->setSliderPosition(value);
                    slider->setValue(value);
                    setTimeText(value);
                    mouseEvent->accept();
                ...
                if (event->type() == QEvent::MouseButtonRelease && m_bAuxProgressDragging) {
                    m_bAuxProgressDragging = false;
                    slider->setSliderPosition(value);
                    slider->setValue(value);
                    if (m_pEngine) {
                        m_pEngine->seekAbsolute(value);
                    }
                    setTimeText(value);
                    mouseEvent->accept();
                    return true;
    ...
    if (m_pAuxProgressBar &&
        (watched == m_pAuxProgressBar ||
         watched == m_pAuxProgressSlider ||
         (m_pAuxProgressSlider && watched == m_pAuxProgressSlider->slider()) ||
         watched == m_pAuxProgressTime)) {
        if (event->type() == QEvent::Enter) {
            if (m_pAuxProgressOpacity) {
                m_pAuxProgressOpacity->setOpacity(0.82);
            }
        } else if (event->type() == QEvent::Leave) {
            const QPoint local = m_pAuxProgressBar->mapFromGlobal(QCursor::pos());
            if (!m_pAuxProgressBar->rect().contains(local)) {
                if (m_pAuxProgressOpacity) {
                    m_pAuxProgressOpacity->setOpacity(0.45);
                }
            }
...
void MainWindow::initMember()
{
    ...
    m_nLastCookie = 0;
    m_nPowerCookie = 0;
    double persistedSpeed = Settings::get().internalOption("play_speed").toDouble();
    if (persistedSpeed < 0.1 || persistedSpeed > 10.0) {
        persistedSpeed = 1.0;
    }
    m_dPlaySpeed = persistedSpeed;
    m_iAngleDelta = 0;
    m_nFullscreenTime = 0;
...
void MainWindow::adjustPlaybackSpeed(ActionFactory::ActionKind actionKind)
{
    ...
    if (m_pEngine->state() != PlayerEngine::CoreState::Idle) {
        ...
        if (actionKind == ActionFactory::ActionKind::AccelPlayback) {
            ...
            m_dPlaySpeed = qMin(10.0, m_dPlaySpeed + 0.1);
        ...
        } else if (qFuzzyCompare(2.5, m_dPlaySpeed)) {
            qDebug() << "qFuzzyCompare(2.5, m_dPlaySpeed)";
            setPlaySpeedMenuChecked(ActionFactory::ActionKind::TowPointFiveTimes);
        } else if (qFuzzyCompare(3.0, m_dPlaySpeed)) {
            qDebug() << "qFuzzyCompare(3.0, m_dPlaySpeed)";
            setPlaySpeedMenuChecked(ActionFactory::ActionKind::Treble);
        }else if (qFuzzyCompare(10.0, m_dPlaySpeed)) {
            qDebug() << "qFuzzyCompare(10.0, m_dPlaySpeed)";
            setPlaySpeedMenuChecked(ActionFactory::ActionKind::TenTimes);
        } else {
...
void MainWindow::setPlaySpeedMenuUnchecked()
{
    ...
    {
        listActs = ActionFactory::get().findActionsByKind(ActionFactory::ActionKind::TowPointFiveTimes);
        auto p = listActs.begin();
        if ((*p)->isChecked()) {
            (*p)->setChecked(false);
        }
    ...
    {
        listActs = ActionFactory::get().findActionsByKind(ActionFactory::ActionKind::Treble);
        auto p = listActs.begin();
        if ((*p)->isChecked()) {
            (*p)->setChecked(false);
        }
    ...
    {
        listActs = ActionFactory::get().findActionsByKind(ActionFactory::ActionKind::TenTimes);
        auto p = listActs.begin();
        if ((*p)->isChecked()) {
            (*p)->setChecked(false);
        }
